...
# --- Setari BLE ---
CHAR_UUID = "0000ff01-0000-1000-8000-00805f9b34fb" # UUID-ul din ESP-IDF
...
class RoverControlGUI:
    def __init__(self, root):
        ...
        # Preluam controlul asincron intr-un thread separat
    ...
    def setup_ui(self):
        ...
        tk.Button(body_frame, text="Usa Stanga (L)", font=font_butoane, bg="#d35400", fg="white", command=lambda: self.trimite_comanda('L')).grid(row=0, column=0, sticky="nsew", padx=3, pady=3)
        tk.Button(body_frame, text="Usa Dreapta (R)", font=font_butoane, bg="#d35400", fg="white", command=lambda: self.trimite_comanda('R')).grid(row=0, column=1, sticky="nsew", padx=3, pady=3)
        tk.Button(body_frame, text="Ciclu Faruri (H)", font=font_butoane, bg="#f1c40f", fg="black", command=lambda: self.trimite_comanda('H')).grid(row=1, column=0, sticky="nsew", padx=3, pady=3)
        tk.Button(body_frame, text="🔍 Gasire Masina (C)", font=font_butoane, bg="#8e44ad", fg="white", command=lambda: self.trimite_comanda('C')).grid(row=1, column=1, sticky="nsew", padx=3, pady=3)
    ...
    def trimite_comanda(self, comanda):
        dict_comenzi = {
            'L': "TOGGLE USA STANGA", 'R': "TOGGLE USA DREAPTA", 'H': "SCHIMBARE MOD FARURI",
        ...
        if ret:
            ...
            if rezultat.multi_hand_landmarks:
                for hand_landmarks in rezultat.multi_hand_landmarks:
                    ...
                    if nr_degete == 0: comanda_curenta = 'S'      # Pumn inchis -> STOP
                    elif degete_ridicate == [0, 1, 0, 0, 0]: comanda_curenta = 'L' # Index ridicat -> Usa Stanga
                    elif degete_ridicate == [0, 1, 1, 0, 0]: comanda_curenta = 'R' # Index + Mijloc -> Usa Dreapta
                    elif nr_degete == 3: comanda_curenta = 'H'    # 3 degete -> Schimbare faruri
                    elif nr_degete == 4: comanda_curenta = 'C'    # 4 degete -> Cautare masina
